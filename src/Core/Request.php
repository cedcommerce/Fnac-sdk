...
abstract class Request implements \FnacSdk\Core\RequestInterface
{
    /**
     * Debug Logging
     * @var $debugMode
    ...
    /**
     * XML Parser
     * @var \FnacSdk\Core\Generator
     */
    public $xml;

    /**
     * Parser
     * @var \FnacSdk\Core\Parser
     */
    public $parser;
     ...
     */
    public function __construct(\FnacSdk\Core\ConfigInterface $config)
    {
    ...
    public function responseParse($response = false, $type = self::FEED_CODE_ITEM_UPDATE, $filePath = '')
    {
        ...
        if ($type && $response) {
            try {
                ...
                if ($this->debugMode) {
                    $this->logger->debug(
                        "FnacSdk\\Core\\Request\\responseParse(): \n Response : \n" . var_export($response, true) .
                        "\n Exception : \n" . var_export($e->getMessage(), true)
                    );
                }
            }
    ...
    public function validateXml($filePath, $xsdPath, $type = 'lmp-item')
    {
        ...
        if ($result === true) {
            if (($feed->schemaValidate($xsdPath))) {
                ...
                if ($this->debugMode) {
                    $this->logger->debug(
                        "FnacSdk\\Core\\Request\\validateXml(): \n" . "\n Errors : \n" . var_export($errorList, true)
                    );
                }
