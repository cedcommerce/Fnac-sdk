...
class Product extends \FnacSdk\Core\Request
{
    /**
    ...
    public function getCategories($params, $forceFetch = false)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "FnacSdk\\Product\\getCategories() : Errors: " . var_export($e->getMessage(), true)
                );
            }
        }
...
//            echo $this->baseDirectory; die;
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "FnacSdk\\Product\\getAttributes() : Errors: " . var_export($e->getMessage(), true)
                );
            }
        }
        return $attributes;
...
//            echo "<pre>"; print_r($product); exit;
    ...
    public function getInventory($skus = [])
    {
        if (!empty($skus) and is_array($skus)) {
            ...
            try {
                ...
                if ($this->debugMode) {
                    $this->logger->addDebug('\FnacSdk\Product\getInventory(): Error: ' . $e->getMessage());
                }
                return false;
            }
        }
    ...
    public function getValueLists($attributeCode, $forceFetch = false, $url = self::GET_ATTRIBUTES_VALUE_LIST)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "FnacSdk\\Product\\getValueLists() : Errors: " . var_export($e->getMessage(), true)
                );
            }
            return $valueLists;
        }
    ...
    public function getOffers($chunkSize = 50, $offset = 0, $url = self::GET_OFFERS)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "FnacSdk\\Product\\getOffers() : Errors: " . var_export($e->getMessage(), true)
                );
            }
            return $offers;
        }
